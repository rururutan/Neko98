...
BOOL CALLBACK DesktopHunter(HWND hWnd, LPARAM lParam)
{
	...
	case ENUM_ALLWINDOWS:
		...
		if (_stricmp(szBuffer, "Progman") == 0) {
	...
	case ENUM_SHELLDEFVIEW:
		GetClassName(hWnd, szBuffer, 25);
		if (_stricmp(szBuffer, "SHELLDLL_DefView") == 0) {
	...
	case ENUM_GETDESKTOP:
		...
		if (_stricmp(szBuffer, "SysListView32") == 0) {
		...
		//check for IE4
		if (_stricmp(szBuffer, "Internet Explorer_Server") == 0) {
			g_hWndDesktop = hWnd;
