...
CNeko::CNeko(char* lpszName)
{
	...
	//set variables
	m_nDX = m_nDY = 0;
	strcpy_s(m_szName, sizeof(m_szName), lpszName);
	m_dwSpeed = 16;
	m_dwIdleSpace = 6;
	...
	*m_szLibname = '\0';
	...
	strcpy_s(m_szSndIdle1, sizeof(m_szSndIdle1), "");
	strcpy_s(m_szSndIdle2, sizeof(m_szSndIdle2), "");
	strcpy_s(m_szSndIdle3, sizeof(m_szSndIdle3), "");
	strcpy_s(m_szSndSleep, sizeof(m_szSndSleep), "");
	strcpy_s(m_szSndAwake, sizeof(m_szSndAwake), "");
	m_dwSndFrequency = 0;
	m_dwScale = 100;

	//build configuration registry key
	char szKey[1024];
	strcpy_s(szKey, sizeof(szKey), szNekoRegKey);
	if (strlen(m_szName) > 0) {
		strcat_s(szKey, sizeof(szKey), "\\");
		strcat_s(szKey, sizeof(szKey), m_szName);
	}
